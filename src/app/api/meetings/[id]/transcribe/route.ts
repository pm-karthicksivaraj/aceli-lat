...
export async function POST(
  ...
  try {
    ...
    // Create PromptHistory entry
    const promptTemplate = "aceli-lat-extraction-v1";
    const inputSnapshot = JSON.stringify({
      meetingId: id,
      lenderName: meeting.lender.institutionName,
      typedNotes: meeting.typedNotes,
      activationAreas: EXTRACTION_TEMPLATES.map((t) => t.area),
    });
    const outputSnapshot = JSON.stringify({
      transcript: transcriptText,
      extractions: extractions.map((e) => ({
        fieldName: e.fieldName,
        activationArea: e.activationArea,
    ...
    await db.promptHistory.create({
      data: {
        ...
        tokenCount: Math.floor(inputSnapshot.length / 4) + Math.floor(outputSnapshot.length / 4),
        duration,
      },
    });
    ...
    // Create audit log for extraction creation
    await db.auditLog.create({
      data: {
        entityType: "Extraction",
        entityId: id,
        action: "Create",
        performedBy: "ai_service",
